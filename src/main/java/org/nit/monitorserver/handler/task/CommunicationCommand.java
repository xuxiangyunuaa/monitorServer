...
public class CommunicationCommand extends AbstractRequestHandler {
...
//    private ByteConversion byteConver =new ByteConversion();
//    private Vertx vertx=Vertx.vertx();
//    private int port=IniUtil.getInstance().getUdpServerPort();
//    private String host=IniUtil.getInstance().getUdpServerHost();
    public static String targetGuid = null;

//    public void EDT_TDR_CFGCTR_MSG(ResponseFactory response,String targetIp){
...
//            result.put("drt_data",TdrMsgBuffer).put("targetIP",targetIp).put("portName",res.result().getRows().get(0).getString("drt_portname"));
//
//            try{
//                switch (eventType){
//                    case 10000:
...
//
//                }
//                mongoClient.insert("communication_record", result, r -> {
//                    if (r.failed()) {
//                        logger.error(String.format("insert: %s, exception: %s", result, Tools.getTrace(r.cause())));
...
//    public JsonObject EDT_DATA_ANALYSIS_CFG(String targetGuid){
//        JsonObject condition = new JsonObject().put("targetGuid",targetGuid);
...
//        mongoClient.findWithOptions("analysisRtn",condition,options,r->{
//            if(r.failed()){
...
//    }
    ...
    public void handle(final RoutingContext routingContext,final Request request) throws IOException {
        ...
        // 获取参数
        Object flagObject = request.getParamWithKey(GlobalConsts.FLAG);
        ...
        if(responseObject.getString("type").equals("error")){
            response.error(responseObject.getInteger("code"),responseObject.getString("message"));
            return;
        }else {
            response.success(responseObject.getJsonObject("result"));
        }
...
////           }
//             EDT_TDR_CFGCTR_MSG(response,targetIp);
//
//
//
//
...
//                    .setMsgType(1)//消息类型
//                    .setIpAddr((String)request.getParamWithKey(GlobalConsts.TARGET_IP)) ;//ip地址
//
...
//
////            System.out.println(Buffer.buffer(udpPacket.build().toByteArray()));
//            byte[] buffer=udpPacket_analysis.build().toByteArray();
////            udpPacket.
//            commandSendsocket.send(Buffer.buffer(udpPacket_analysis.build().toByteArray()),port,host,asyncResult->{
//                    if(asyncResult.succeeded()){
//                        sendCommandFuture.complete();
//                        logger.info("Command send success!");
//                    }
...
//
//            sendCommandFuture.setHandler(asyn->{
//                    EDT_DATA_ANALYSIS_CFG(targetGuid);
//            });
//
//
//            }
            ...
            //获取命令配置
//            if(params.getJsonObject("get_cmd").getJsonObject("getCmd").getValue("switch").equals("true")){
//                JsonObject getCmdParams=params.getJsonObject("get_cmd").getJsonObject("getCmd");
//                //获取命令配置
//
//                HdpMsg.ERR_HEAD_GET.Builder head=HdpMsg.ERR_HEAD_GET.newBuilder();//获取故障记录信息头部
//                HdpMsg.ERR_ENTRY_GET.Builder entries=HdpMsg.ERR_ENTRY_GET.newBuilder();//故障记录信息条目
//                HdpMsg.ERR_GET_INFO.Builder err_getInfo=HdpMsg.ERR_GET_INFO.newBuilder();//故障记录获取配置信息
//                HdpMsg.IMG_GET_INFO.Builder img_getInfo=HdpMsg.IMG_GET_INFO.newBuilder();//获取空间数据信息
//                HdpMsg.TIMESYN_GET_INFO.Builder time_getInfo=HdpMsg.TIMESYN_GET_INFO.newBuilder();//获取同步信息
//
//                //获取故障记录信息
//                if(getCmdParams.getJsonObject("errGetInfo").getValue("switch").equals("true")){
//                    JsonObject getErrGetParams=getCmdParams.getJsonObject("errGetInfo");
//                    err_getInfo.setErrType(Integer.parseInt((String) getCmdParams.getJsonObject("errGetInfo").getValue(GlobalConsts.ERRTYPE)));
////                    System.out.println(getErrGetParams);
//                    if(getErrGetParams.getJsonObject("head").getValue("switch").equals("true")){
//                        head.setPartId(Integer.parseInt((String) getErrGetParams.getJsonObject("head").getValue(GlobalConsts.PART_ID)));//头部信息所在分区
//                        err_getInfo.setHead(head);
//                    }
//                    if(getErrGetParams.getJsonObject("entries").getValue("switch").equals("true")){
//                        entries.setStartEntry(Integer.parseInt((String) getErrGetParams.getJsonObject("entries").getValue(GlobalConsts.START_ENTRY)));
//                        entries.setPartId(Integer.parseInt((String) getErrGetParams.getJsonObject("entries").getValue(GlobalConsts.PART_ID)));
//                        entries.setReqNum(Integer.parseInt((String) getErrGetParams.getJsonObject("entries").getValue(GlobalConsts.REQ_NUM)));
//                        err_getInfo.setEntries(entries);
//                    }
//                    getCmd.setErrGetInfo(err_getInfo);
//                }
//                //获取内存镜像信息
//                if(getCmdParams.getJsonObject("imgGetInfo").getValue("switch").equals("true")){
//                    img_getInfo.setTid(Integer.parseInt((String) getCmdParams.getJsonObject("imgGetInfo").getValue(GlobalConsts.TID)));
//                    getCmd.setImgGetInfo(img_getInfo);
//                }
//                //同步信息获取
//                if(getCmdParams.getJsonObject("timeGetInfo").getValue("switch").equals("true")){
//                    time_getInfo.setTimeTag(Integer.parseInt(getCmdParams.getJsonObject("timeGetInfo").getValue(GlobalConsts.TIME_TAG).toString()
//                            .substring(2),16));
//                    getCmd.setTimeGetInfo(time_getInfo);
//                }
//                edt_tdr_cfgctr_msg.setGetCmd(getCmd);
//            }

            //配置命令格式
//            if(params.getJsonObject("cfg_cmd").getJsonObject("cfgCmd").getValue("switch").equals("true")){
//                JsonObject cfgCmdParams=params.getJsonObject("cfg_cmd").getJsonObject("cfgCmd");
//                HdpMsg.SYS_DET_CFG.Builder sysDet_cfg=HdpMsg.SYS_DET_CFG.newBuilder();//系统检测事件配置
//                HdpMsg.RCD_MGR_CFG.Builder rcdMgr_cfg=HdpMsg.RCD_MGR_CFG.newBuilder();//记录信息管理模块配置信息
//
//                //系统检测事件配置
//                //时间资源
//                HdpMsg.TIMESRC_CFG.Builder timeSrc_cfg=HdpMsg.TIMESRC_CFG.newBuilder();//时间资源
...
//
//                //系统检测事件
//                if(cfgCmdParams.getJsonObject("sysDetCfg").getValue("switch").equals("true")){
//
//                    JsonObject timeSrcCfg=cfgCmdParams.getJsonObject("sysDetCfg").getJsonObject("timeSrcCfg");//时间资源信息
//                    if(timeSrcCfg.getValue("switch").equals("true")){
//                        if(timeSrcCfg.getJsonObject("tskSwtInfo").getValue("switch").equals("true")){
//                            cfg_tskswt.setTskSwId(Integer.parseInt((String) timeSrcCfg.getJsonObject("tskSwtInfo").getValue(GlobalConsts.TSKSWID)));
//                            timeSrc_cfg.setTskSwtInfo(cfg_tskswt);
//                            sysDet_cfg.setTimeSrcCfg(timeSrc_cfg);
//                        }
//                    }
//                    //空间资源信息
//                    if(cfgCmdParams.getJsonObject("sysDetCfg").getJsonObject("spaceSrcCfg").getValue("switch").equals("true")){
//                        JsonObject spaceSrcCfg=cfgCmdParams.getJsonObject("sysDetCfg").getJsonObject("spaceSrcCfg");//空间资源信息
//                        if(spaceSrcCfg.getJsonObject("taskSpawnInfo").getValue("switch").equals("true")){
//                            cfg_taskspawn.setTskSpId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("taskSpawnInfo").getValue(GlobalConsts.TSKSPID)));
//                            spacesrc_cfg.setTaskSpawnInfo(cfg_taskspawn);
//                        }
//                        if(spaceSrcCfg.getJsonObject("taskDeleteInfo").getValue("switch").equals("true")){
//                            cfg_taskdelete.setTskDtId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("taskDeleteInfo").getValue(GlobalConsts.TSKDTID)));
//                            spacesrc_cfg.setTaskDeleteInfo(cfg_taskdelete);
//                        }
//                        if(spaceSrcCfg.getJsonObject("ioCreateInfo").getValue("switch").equals("true")){
//                            cfg_iocreate.setIoCtId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("ioCreateInfo").getValue(GlobalConsts.IOCTID)));
//                            spacesrc_cfg.setIoCreateInfo(cfg_iocreate);
//                        }
//                        if(spaceSrcCfg.getJsonObject("ioDeleteInfo").getValue("switch").equals("true")){
//                            cfg_iodelete.setIoDtId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("ioDeleteInfo").getValue(GlobalConsts.IODTID)));
//                            spacesrc_cfg.setIoDeleteInfo(cfg_iodelete);
//                        }
//                        if(spaceSrcCfg.getJsonObject("semCreateInfo").getValue("switch").equals("true")){
//                            cfg_semcreate.setSemCtId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("semCreateInfo").getValue(GlobalConsts.SEMCTID)));
//                            spacesrc_cfg.setSemCreateInfo(cfg_semcreate);
//                        }
//                        if(spaceSrcCfg.getJsonObject("semDeleteInfo").getValue("switch").equals("true")){
//                            cfg_semdelete.setSemDtId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("semDeleteInfo").getValue(GlobalConsts.SEMDTID)));
//                            spacesrc_cfg.setSemDeleteInfo(cfg_semdelete);
//                        }
//                        if(spaceSrcCfg.getJsonObject("wdCreateInfo").getValue("switch").equals("true")){
//                            cfg_wdcreate.setWdCtId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("wdCreateInfo").getValue(GlobalConsts.WDCTID)));
//                            spacesrc_cfg.setWdCreateInfo(cfg_wdcreate);
//                        }
//                        if(spaceSrcCfg.getJsonObject("wdDeleteInfo").getValue("switch").equals("true")){
//                            cfg_wddelete.setWdDtId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("wdDeleteInfo").getValue(GlobalConsts.WDDTID)));
//                            spacesrc_cfg.setWdDeleteInfo(cfg_wddelete);
//                        }
//                        if(spaceSrcCfg.getJsonObject("msgQCreateInfo").getValue("switch").equals("true")){
//                            cfg_msgqcreate.setMsgQCtId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("msgQCreateInfo").getValue(GlobalConsts.MSGQCTID)));
//                            spacesrc_cfg.setMsgQCreateInfo(cfg_msgqcreate);
//                        }
//                        if(spaceSrcCfg.getJsonObject("msgQDeleteInfo").getValue("switch").equals("true")){
//                            cfg_msgqdelete.setMsgQDtId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("msgQDeleteInfo").getValue(GlobalConsts.MSGQDTID)));
//                            spacesrc_cfg.setMsgQDeleteInfo(cfg_msgqdelete);
//                        }
//                        if(spaceSrcCfg.getJsonObject("mallocInfo").getValue("switch").equals("true")){
//                            cfg_malloc.setMallocId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("mallocInfo").getValue(GlobalConsts.MALLOCID)));
//                            spacesrc_cfg.setMallocInfo(cfg_malloc);
//                        }
//                        if(spaceSrcCfg.getJsonObject("memPartCreateInfo").getValue("switch").equals("true")){
//                            cfg_mempartcreate.setMemPCreateId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("memPartCreateInfo").getValue(GlobalConsts.MEMPCREATEID)));
//                            spacesrc_cfg.setMemPartCreateInfo(cfg_mempartcreate);
//                        }
//                        if(spaceSrcCfg.getJsonObject("freeInfo").getValue("switch").equals("true")){
//                            cfg_free.setFreeId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("freeInfo").getValue(GlobalConsts.FREEID)));
//                            spacesrc_cfg.setFreeInfo(cfg_free);
//                        }
//                        if(spaceSrcCfg.getJsonObject("stackInfo").getValue("switch").equals("true")){
//                            cfg_stack.setStackId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("stackInfo").getValue(GlobalConsts.STACKID)));
//                            spacesrc_cfg.setStackInfo(cfg_stack);
//                        }
//                        if(spaceSrcCfg.getJsonObject("pdMemInfo").getValue("switch").equals("true")){
//                            cfg_pdmem.setPdMemId(Integer.parseInt((String) spaceSrcCfg.getJsonObject("pdMemInfo").getValue(GlobalConsts.PDMEMID)));
//                            cfg_pdmem.setPeriod(Integer.parseInt((String) spaceSrcCfg.getJsonObject("pdMemInfo").getValue(GlobalConsts.PERIOD)));
//                            spacesrc_cfg.setPdMemInfo(cfg_pdmem);
//                        }
//                        sysDet_cfg.setSpaceSrcCfg(spacesrc_cfg);
//                    }
//
//                    //系统事件信息
//                    if(cfgCmdParams.getJsonObject("sysDetCfg").getJsonObject("sysEvtCfg").getValue("switch").equals("true")){
//                        JsonObject sysEvtCfg=cfgCmdParams.getJsonObject("sysDetCfg").getJsonObject("sysEvtCfg");//系统事件信息
//                        if(sysEvtCfg.getJsonObject("isrInfo").getValue("switch").equals("true")){
//                            cfg_isr.setIsrId(Integer.parseInt((String) sysEvtCfg.getJsonObject("isrInfo").getValue(GlobalConsts.ISRID)));
//                            sysevt_cfg.setIsrInfo(cfg_isr);
//                        }
//                        if(sysEvtCfg.getJsonObject("eptInfo").getValue("switch").equals("true")){
//                            cfg_ept.setEptId(Integer.parseInt((String) sysEvtCfg.getJsonObject("eptInfo").getValue(GlobalConsts.HMID)));
//                            sysevt_cfg.setEptInfo(cfg_ept);
//                        }
//                        if(sysEvtCfg.getJsonObject("edrInfo").getValue("switch").equals("true")){
//                            cfg_edr.setEdrId(Integer.parseInt((String) sysEvtCfg.getJsonObject("edrInfo").getValue(GlobalConsts.EDRID)));
//                            sysevt_cfg.setEdrInfo(cfg_edr);
//                        }
//                        sysDet_cfg.setSysEvtCfg(sysevt_cfg);
//                    }
//
//                    //数据通信信息
//                    if(cfgCmdParams.getJsonObject("sysDetCfg").getJsonObject("dataComCfg").getValue("switch").equals("true")){
//                        JsonObject dataComCfg=cfgCmdParams.getJsonObject("sysDetCfg").getJsonObject("dataComCfg");//数据通信信息
////                        if(dataComCfg.getJsonObject("OSACSENDInfo").getValue("switch").equals("true")){
////                            cfg_osac_send.setOSACSndId(Integer.parseInt((String) dataComCfg.getJsonObject("OSACSENDInfo").getValue(GlobalConsts.OSACID)));
////                            datacom_cfg.setOsacSendInfo(cfg_osac_send);
////                        }
////                        if(dataComCfg.getJsonObject("OSACRECVInfo").getValue("switch").equals("true")){
////                            cfg_osac_recv.setOSACRcvId(Integer.parseInt((String) dataComCfg.getJsonObject("OSACRECVInfo").getValue(GlobalConsts.OSACID)));
////                            datacom_cfg.setOsacRecvInfo(cfg_osac_recv);
////                        }
//                        if(dataComCfg.getJsonObject("msgQSndInfo").getValue("switch").equals("true")){
//                            cfg_msgqsnd.setMsgQSndId(Integer.parseInt((String) dataComCfg.getJsonObject("msgQSndInfo").getValue(GlobalConsts.MSGQSNDID)));
//                            datacom_cfg.setMsgQSndInfo(cfg_msgqsnd);
//                        }
//                        if(dataComCfg.getJsonObject("msgQRecInfo").getValue("switch").equals("true")){
//                            cfg_msgqrec.setMsgQRecId(Integer.parseInt((String) dataComCfg.getJsonObject("msgQRecInfo").getValue(GlobalConsts.MSGQRECID)));
//                            datacom_cfg.setMsgQRecInfo(cfg_msgqrec);
//                        }
//                        sysDet_cfg.setDataComCfg(datacom_cfg);
//                    }
//                }
//
//                ////记录信息管理模块
//                if(cfgCmdParams.getJsonObject("rcdMgrCfg").getValue("switch").equals("true")){
//                    JsonObject rcdMgrCfgParams=cfgCmdParams.getJsonObject("rcdMgrCfg");
//                    rcd_mgr_cfg.setRcdMgrId(Integer.parseInt((String) rcdMgrCfgParams.getValue(GlobalConsts.RCDMGRID)));
//                    rcd_mgr_cfg_sub.setMin(Integer.parseInt((String) rcdMgrCfgParams.getJsonObject("subInfo").getValue(GlobalConsts.MIN)))
//                            .setMax(Integer.parseInt((String) rcdMgrCfgParams.getJsonObject("subInfo").getValue(GlobalConsts.MAX)))
//                            .setBuffSize(Integer.parseInt((String) rcdMgrCfgParams.getJsonObject("subInfo").getValue(GlobalConsts.BUFFSIZE)))
//                            .setMode(Integer.parseInt((String) rcdMgrCfgParams.getJsonObject("subInfo").getValue(GlobalConsts.MODE)))
//                            .setOption(Integer.parseInt((String) rcdMgrCfgParams.getJsonObject("subInfo").getValue(GlobalConsts.OPTION)))
//                            .setBuffMgrPri(Integer.parseInt((String) rcdMgrCfgParams.getJsonObject("subInfo").getValue(GlobalConsts.BUFFMGRPRI)))
//                            .setBuffUpPri(Integer.parseInt((String) rcdMgrCfgParams.getJsonObject("subInfo").getValue(GlobalConsts.BUFFUPPRI)));
//                    rcd_mgr_cfg.setSubInfo(rcd_mgr_cfg_sub);
//                }
//
//                cfg_cmd_st.setSysDetCfg(sysDet_cfg).setRcdMgrCfg(rcd_mgr_cfg);
//            }

            //控制命令格式
//            if(params.getJsonObject("ctr_cmd").getJsonObject("ctrCmd").getValue("switch").equals("true")){
//                ctr_cmd_st.setCfgCtrId(Integer.parseInt((String) params.getJsonObject("ctr_cmd").getJsonObject("ctrCmd").getValue(GlobalConsts.CFGCTRID)));
//                ctr_cmd_st.setRunFlag(Integer.parseInt((String) params.getJsonObject("ctr_cmd").getJsonObject("ctrCmd").getValue(GlobalConsts.RUNFLAG)));
//            }

//            整体命令构建
//            edt_tdr_cfgctr_msg.setGetCmd(getCmd).setCfgCmd(cfg_cmd_st).setCtrCmd(ctr_cmd_st);
//            HdpMsg.EDT_TDR_CFGCTR_MSG edttdrcfgctrmsg=edt_tdr_cfgctr_msg.build();
////            AnalysisCfg.EDT_DATA_ANALYSIS_CFG edtdataanalysiscfg = edt_data_analysis_cfg.build();
//            System.out.println(edttdrcfgctrmsg);
////            System.out.println(edtdataanalysiscfg);
...
//        else{
//            Object targetIP = request.getParamWithKey(GlobalConsts.TARGET_IP);
//            Object jobId=request.getParamWithKey(AcquisitionConfigConsts.JOBID);
//
//            //1.构建命令
//
//            //9.转换为字节数组，发送数据包
//            DatagramSocket socket = vertx.createDatagramSocket(new DatagramSocketOptions());
//            socket.send((Buffer) cfgIns, port, host,asyncResult -> {
//                System.out.println("Send succeeded? " + asyncResult.succeeded());
//                //DatagramSocket listenSocket = vertx.createDatagramSocket(new DatagramSocketOptions());
//            });
//
//            Future<byte[]> feedBack=Future.future();
//            DatagramSocket socket1 = vertx.createDatagramSocket(new DatagramSocketOptions());
//            socket1.ListenAcquisition(port, "127.0.0.1", asyncResultRe -> {
...
//                    feedBack.complete(packet.data().getBytes()); //获取反馈信息
//                    JsonObject result1=new JsonObject();
//                    result1.put("data",packet.data().toString());
//                    response.success(result1);  //响应前端数据
//                    logger.info(String.format("exception:命令%s数据返回成功",packet.data().toString()));
...
//            });
//
//            feedBack.setHandler(res->{
//                byte[] feedBackData=res.result();
//                try {
//                    FeedBack.FB_MSG_HEADER msg_header=FeedBack.FB_MSG_HEADER.parseFrom(feedBackData);
//
//                    //数据库操作
////                JsonObject params=new JsonObject();
////                String sql="";
////                mySQLClient.updateWithParams(sql,params,Sqlres->{
////
////                });
//
//                } catch (InvalidProtocolBufferException e) {
