...
//                                String timeStampCurrent = format.format(calendar.getTime());
                                ...
                                //数据分析结果数据库
                                AnalysisRtn.EDT_DATA_ANALYSIS_RTN data_analysis_rtn=AnalysisRtn.EDT_DATA_ANALYSIS_RTN.parseFrom(udppacket.getBuffers());
...
//                                    dataanalysistn.put("evt_mem_effect_rtn",evtmemeffectrtn);
                                    ...
                                    JsonObject searchObject = new JsonObject().put("targetIP",targetIp).put("evtId",99990).put("type","ana");
                                    String collectionName = targetIp+String.valueOf(99990);
                                    mongoClient.find("targetExecuting",searchObject,re->{
                                        if(re.failed()){
                                            logger.error(String.format("search targetEvtId: %s 查找失败", Tools.getTrace(re.cause())));
                                            return;
                                        }else if(re.result().size() == 0){

                                            searchObject.put("id",collectionName);
                                            mongoClient.insert("targetExecuting",searchObject,rs->{
                                                if(rs.failed()){
...
//                                    }
                                    ...
                                    JsonObject searchObject = new JsonObject().put("targetIP",targetIp).put("evtId",99991).put("type","ana");
...
//                                    insertMysql(sql,params);
                                ...
                                if(data_analysis_rtn.hasFuncTimeuseStaticRtn()){
                                    ...
                                    JsonObject searchObject = new JsonObject().put("targetIP",targetIp).put("evtId",99992).put("type","ana");
...
//                                    }
                                    ...
                                    JsonObject searchObject = new JsonObject().put("targetIP",targetIp).put("evtId",99993).put("type","ana");
                                    ...
                                    mongoClient.find("targetExecuting",searchObject,re->{
                                        if(re.failed()){
                                            ...
                                            searchObject.put("id",colectionName);
                                            mongoClient.insert("targetExecuting",searchObject,rs->{
                                                if(rs.failed()){
                                                    logger.error(String.format("new targetIPEvtId insert exception: %s", Tools.getTrace(rs.cause())));
                                                    return;
...
//                                    JsonArray taskruntimeinfo = new JsonArray();
                                    ...
                                    JsonObject searchObjet = new JsonObject().put("targetIP",targetIp).put("evtId",99994).put("type","ana");
                                    ...
                                    mongoClient.find("targetEvtId",searchObjet,re->{
                                        if(re.failed()){
                                            ...
                                            searchObjet.put("id",collectionName);
...
//                                    }
                                    ...
                                    JsonObject searchObject  = new JsonObject().put("targetIP",targetIp).put("evtId",99995).put("type","ana");
